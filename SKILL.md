...
- Prefer empirical verification you can delegate: "write a failing test that proves the bug, then fix it" beats an opinion pass at any tier. After a multi-fix audit, your findings table IS the test brief: delegate regression-test typing by pointing at it ("one test pinning each finding; the failure scenario is the test") plus style and wiring facts. If you catch yourself writing exact per-test assertions into the brief, the floor rule has flipped — the assertions ARE the tests, so type them yourself.
- **Audit backstop:** when the engagement is an audit and you did the auditing yourself, buy one cheap insurance pass: a single `exec`-model fresh-eyes sweep instructed to list candidate contract violations (`file:line` + quote; uncertain candidates welcome). It needs nothing from you, so launch it in parallel at the START of your own audit — never as a serial round afterward — and adjudicate every candidate by reading the cited code. One extra launch costs cents; a missed finding costs the engagement.
- Delegation rounds are turns, and every turn re-reads your whole context at cache rates. Keep rounds few and fat rather than many and thin: batch independent launches, and never serialize a read-only agent behind work it doesn't depend on.
...
- Build a full feature (design + logic + UI) → mostly yourself: you must read the conventions to design against them, so orientation scouts are overhead, and the design/logic/UI are taste. Delegate the clearly-separable mechanical chunks (test typing against your findings/contract, bulk integration edits) and expect near-zero savings beyond that. Near-zero is CORRECT on taste-led work — do not manufacture delegation to feel efficient.
